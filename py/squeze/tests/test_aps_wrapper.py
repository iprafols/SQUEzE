...
class TestApsWrapper(AbstractTest):
    ...
    def setUp(self):
        ...
        """
        self.in_files = [
            "{}/data/stacked_1004074.fit".format(THIS_DIR),
        ...
        self.redrock_templates = "{}/data/redrock-templates/rrtemplate-qso.fits".format(
            THIS_DIR)
        ...
        self.redrock_archetypes = "{}/data/redrock-quasar-archetypes".format(
            THIS_DIR)
        ...
        self.out_candidates_file = self.out_file.replace(
            ".fits", "_squeze_candidates.fits")
    ...
    def test_aps_wrapper(self):
        ...
        command = ["aps_squeze.py", '--infiles'] + self.in_files
        command += [
            '--aps_ids',
            'None',
            '--targsrvy',
            ...
            'True',
            '--templates',
            self.redrock_templates,
            '--srvyconf',
            self.srvyconf,
            '--archetypes',
            self.redrock_archetypes,
            '--outpath',
            "{}/results/".format(THIS_DIR),
            '--headname',
            'test',
            '--zall',
            'True',
            ...
            '2',
            '--model',
            '{}/data/candidates_boss_test1_nopred_model.json'.format(THIS_DIR),
            '--prob_cut',
            '0.3',
            '--output_catalogue',
            self.out_file,
