...
from squeze.utils import (save_json, deserialize, load_json, verboseprint,
                          quietprint)
...
def main(cmdargs):
    ...
    parser.add_argument(
        ...
        help="""Name of the folder containg the spectra to process. In
                            ...
                            number.""")
    parser.add_argument("--rebin-pixels-width",
                        type=float,
                        default=0,
                        help="""Width of the new pixel (in Angstroms).""")
    parser.add_argument("--extend-pixels",
                        type=float,
                        default=0,
    ...
    parser.add_argument(
        ...
        help="""Margin used in the masking. Wavelengths separated to
                            ...
                            will be masked""")
    parser.add_argument("--single-plate",
                        type=int,
                        required=False,
                        default=0,
    ...
    if args.forbidden_wavelengths is not None:
        ...
        for i in range(0, len(args.forbidden_wavelengths), 2):
            try:
                aux.append((args.forbidden_wavelengths[i],
                            args.forbidden_wavelengths[i + 1]))
            except IndexError:
                verboseprint(
                    "--forbidden-wavelengths must have an even number of elements.\n Stopping..."
                )
                return
    ...
    plate_list = plate_list_hdu[1].data["plate"][np.where(
    ...
    if args.qso_dataframe is not None:
        if ((args.qso_cat is not None) or (args.qso_specid is not None) or
            ...
            parser.error(
                "options --qso-cat, --qso-specid, and --qso-ztrue "
        ...
        if (args.qso_cat is None) or (args.qso_cols is None) or (
                args.qso_specid is None) or (args.qso_ztrue is None):
        ...
        quasar_catalogue = QuasarCatalogue(
            config["quasar catalogue"]).quasar_catalogue
    ...
    if "CLASS_PERSON" not in quasar_catalogue.columns:
        raise Error(
            "Quasar catalogue does not contain the column 'CLASS_PERSON'" +
        ...
        for index in quasar_catalogue[quasar_catalogue["PLATE"] == plate].index:
            ...
            if args.confident_only:
                if (z_conf_person != 3):
                    continue
                if not (
                    (boss_target1 & 2**40 > 0) | (boss_target1 & 2**41 > 0) |
                    (boss_target1 & 2**42 > 0) | (boss_target1 & 2**43 > 0) |
                    (boss_target1 & 2**44 > 0) | (boss_target1 & 2**10 > 0) |
                    (boss_target1 & 2**11 > 0) | (boss_target1 & 2**12 > 0) |
                    (boss_target1 & 2**13 > 0) | (boss_target1 & 2**14 > 0) |
                    (boss_target1 & 2**15 > 0) | (boss_target1 & 2**16 > 0) |
                    (boss_target1 & 2**17 > 0) | (boss_target1 & 2**18 > 0) |
                    (boss_target1 & 2**19 > 0) | (eboss_target0 & 2**10 > 0) |
                    (eboss_target0 & 2**11 > 0) | (eboss_target0 & 2**12 > 0) |
                    (eboss_target0 & 2**13 > 0) | (eboss_target0 & 2**14 > 0) |
                    (eboss_target0 & 2**15 > 0) | (eboss_target0 & 2**16 > 0) |
                    (eboss_target0 & 2**17 > 0) | (eboss_target0 & 2**18 > 0) |
                    (eboss_target0 & 2**20 > 0) | (eboss_target0 & 2**22 > 0) |
                    (eboss_target0 & 2**30 > 0) | (eboss_target0 & 2**31 > 0) |
                    (eboss_target0 & 2**33 > 0) | (eboss_target0 & 2**34 > 0) |
                    (eboss_target0 & 2**35 > 0) | (eboss_target0 & 2**40 > 0)):
                    continue
            metadata = {}
            for column in quasar_catalogue.columns:
                if column != "LOADED":
                    metadata[column] = entry[column]
            spectrum_file = "spec-{:04d}-{:05d}-{:04d}.fits".format(
                plate, entry["MJD"].astype(int), entry["FIBERID"].astype(int))

            # add spectra to list
            try:
                spectra.append(
                    BossSpectrum(
                        "{}{}".format(folder, spectrum_file),
                        metadata, (masklambda, args.margin),
