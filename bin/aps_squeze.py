...
def write_results(zbest, candidates_df, args):
    ...
    candidates_df = candidates_df[(~candidates_df["DUPLICATED"])]

    # sanity check: apply probability cut
    candidates_df = candidates_df[(candidates_df["PROB"] >= args.prob_cut)]

    # then create the final catalogue
    aps_ids = candidates_df["APS_ID"]
    cnames = candidates_df["CNAME"]
    targids = candidates_df["TARGID"]
...
def main(options=None, comm=None):
    ...
    columns = [
        fits.Column(name="TARGETID",
                    format="I",
                    array=candidates_df["APS_ID"]),
        fits.Column(name="Z",
                    format="D",
                    array=candidates_df["Z_TRY"]),
        fits.Column(name="SIGMA",
                    format="D",
                    array=np.ones(candidates_df.shape[0])*z_precision),
    ]
    ...
    if aps_ids is None:
        aps_ids = list(candidates_df["APS_ID"])
    else:
        for aps_id in aps_ids:
            if not apd_id in candidates_df["APS_ID"]:
                aps_ids.pop(aps_id)
    del columns, hdu
